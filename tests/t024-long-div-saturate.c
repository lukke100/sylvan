...
int main(void)
{
	assert(sy_ldiv_saturate( 1, 1, 0) == 1);
	assert(sy_ldiv_saturate( 1, 0, 0) == LONG_MAX);
	assert(sy_ldiv_saturate(-1, 0, 0) == LONG_MIN);

	assert(sy_ldiv_saturate(LONG_MAX,  2, 0) == LONG_MAX);
	assert(sy_ldiv_saturate(LONG_MAX, -2, 0) == LONG_MIN);
	assert(sy_ldiv_saturate(LONG_MAX,  0, 0) == LONG_MAX);
	assert(sy_ldiv_saturate(LONG_MIN,  2, 0) == LONG_MIN);
	assert(sy_ldiv_saturate(LONG_MIN, -2, 0) == LONG_MAX);
	assert(sy_ldiv_saturate(LONG_MIN,  0, 0) == LONG_MIN);
